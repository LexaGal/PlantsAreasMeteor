Router.configure({
    ...
    waitOn: function () {
        if (Session.get("UserNET") != null) {
            return [
                Meteor.subscribe('notifications', Session.get("UserNET")._id),
...
PlantsareasListController = RouteController.extend({
    ...
    onBeforeAction: function () {
        if (Session.get("UserNET") != null) {
            this.plantsareasSub = Meteor.subscribe('plantsareas', Session.get("UserNET")._id, this.findOptions());
        }
        this.next();
    },
...
Router.map(function () {
    ...
    //this.route('plantsareaDelete', {
    //    path: '/plantsareaDelete/:id',
    //    waitOn: function () {
    //        return [
    //            Meteor.subscribe('singlePlantsarea')
    //        ];
    //    }
    //});

    //this.route('plantsareaEdit', {
    //    path: '/plantsareaEdit/:id',
    //    waitOn: function () {
    //        return [
    //            Meteor.subscribe('singlePlantsarea')
    //        ];
    ...
    this.route('plantsareaPage', {
        ...
        data: function () {
            Session.set("plantsareaId", this.params._id);
    ...
    this.route('sensorEdit', {
        path: '/sensors/:_id/edit',
        waitOn: function () {
            return [
                Meteor.subscribe('singleSensor', this.params._id)
            ];
        },
        data: function () {
